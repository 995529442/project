        ...
        <form class="layui-form" action="{{ route('cater.orders.index') }}" method="get">
            ...
            <div class="layui-form-item" style="display:inline-block;margin-left:-40px;">
                ...
                <div class="layui-input-block">
                    <input type="text" name="batchcode" id="batchcode" autocomplete="off" class="layui-input"
                           value="{{$batchcode}}">
                </div>
            </div>
        ...
        <div class="layui-form">
            <table class="layui-table" lay-size="sm">
                ...
                @foreach($order_info as $v)
                    <tr>
                        <td>{{$v->order_id}}</td>
                        <td><a href="{{ route('cater.orders.orderGoods',['order_id'=>$v->order_id]) }}"
                               style="color:#1E9FFF;">{{$v->batchcode}}</a></td>
                        <td>{{$v->weixin_name}}</td>
                        <td>{{$v->phone}}</td>
                        <td>
                            @if($v->type == 1)
                        ...
                        <td>
                            @if($v->status == 0 && $v->pay_type == 0)
                                <button class="layui-btn layui-btn-normal layui-btn-sm"
                                        onclick="operate({{$v->order_id}},'cancel')">取消订单
                                </button>
                            @elseif($v->status == 1 && $v->pay_type == 1)
                                <button class="layui-btn layui-btn-normal layui-btn-sm"
                                        onclick="operate({{$v->order_id}},'accept')">接单
                                </button>
                                <button class="layui-btn layui-btn-normal layui-btn-sm"
                                        onclick="operate({{$v->order_id}},'reject')">拒单
                                </button>
                            @elseif($v->status == 2 && $v->pay_type == 1)
                                <button class="layui-btn layui-btn-normal layui-btn-sm"
                                        onclick="operate({{$v->order_id}},'send')">配送
                                </button>
                            @elseif($v->status == 3 && $v->pay_type == 1)
                                <button class="layui-btn layui-btn-normal layui-btn-sm"
                                        onclick="operate({{$v->order_id}},'confirm_send')">配送完成
                                </button>
                            @elseif($v->status == 4 && $v->pay_type == 1)
                                <button class="layui-btn layui-btn-normal layui-btn-sm"
                                        onclick="operate({{$v->order_id}},'confirm')">完成
                                </button>
                            @elseif($v->status == 6 && $v->pay_type == 1)
                                <button class="layui-btn layui-btn-normal layui-btn-sm"
                                        onclick="operate({{$v->order_id}},'confirm_refund')">确认退款
                                </button>
                                <button class="layui-btn layui-btn-normal layui-btn-sm"
                                        onclick="operate({{$v->order_id}},'reject_refund')">拒绝退款
                                </button>
                            @endif
                        </td>
                    </tr>
